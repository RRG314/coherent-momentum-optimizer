...
def run_magneto_gpu_smoke(config: dict[str, Any]) -> pd.DataFrame:
    ...
    smoke_config.setdefault(
        "smoke_optimizers",
        ["magneto_hamiltonian_adam", "magneto_hamiltonian_adam_improved", "adamw", "rmsprop", "sgd_momentum"],
    )
...
def run_magneto_gpu_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "auto")))
    device_info = _device_summary(device)
...
def run_magneto_gpu_cnn_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "auto")))
    device_info = _device_summary(device)
    tuning_config = dict(config)
...
def run_magneto_gpu_stress_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "auto")))
    device_info = _device_summary(device)
    tuning_config = dict(config)
...
def run_magneto_gpu_multitask_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "auto")))
    device_info = _device_summary(device)
    tuning_config = dict(config)
...
def run_magneto_gpu_ablation(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "auto")))
    ...
    variants = [
        {"variant_name": "current_magneto_full", "optimizer_name": "magneto_hamiltonian_adam", "overrides": {}},
        {"variant_name": "improved_balanced", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"preset": "balanced"}},
        {"variant_name": "improved_standard_safe", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"preset": "standard_safe"}},
        {"variant_name": "improved_stress_specialist", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"preset": "stress_specialist"}},
        {"variant_name": "improved_cnn_safe", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"preset": "cnn_safe"}},
        {"variant_name": "real_hamiltonian_adam", "optimizer_name": "real_hamiltonian_adam", "overrides": {}},
        {"variant_name": "magneto_adam_only", "optimizer_name": "magneto_adam", "overrides": {}},
        {
            "variant_name": "projection_under_conflict_only",
            "optimizer_name": "magneto_hamiltonian_adam_improved",
        ...
        {"variant_name": "fixed_mass", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"mass_mode": "fixed", "fixed_mass": 1.0}},
        {"variant_name": "adaptive_mass", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"mass_mode": "adaptive"}},
        {"variant_name": "conv_safe_off", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"conv_safe_mode": False}},
        {"variant_name": "conv_safe_on", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"conv_safe_mode": True}},
        {"variant_name": "diagnostics_full", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"diagnostics_every_n_steps": 1}},
        {"variant_name": "diagnostics_throttled", "optimizer_name": "magneto_hamiltonian_adam_improved", "overrides": {"diagnostics_every_n_steps": 8}},
        ...
        for variant in variants:
            for seed in seeds:
                row = _train_single_run(
                    suite_name="magneto_gpu_ablation",
                    task_name=task_name,
...
def export_magneto_gpu_report(output_dir: str | Path) -> dict[str, Any]:
    ...
    win_frames = []
    for optimizer_name in ["magneto_hamiltonian_adam", "magneto_hamiltonian_adam_improved"]:
    ...
    if not trace_frame.empty:
        _plot_metric(
            ...
            optimizers=["magneto_hamiltonian_adam", "magneto_hamiltonian_adam_improved", "adamw", "rmsprop"],
            event="val",
        )
        _plot_metric(
            trace_frame,
        ...
        _plot_metric(
            ...
            optimizers=["magneto_hamiltonian_adam", "magneto_hamiltonian_adam_improved", "real_hamiltonian_adam"],
        )
        _plot_metric(
            trace_frame,
            ...
            optimizers=["magneto_hamiltonian_adam", "magneto_hamiltonian_adam_improved"],
        )
        _plot_metric(
            trace_frame,
            output_path=figure_dir / "projection_activation_curves.png",
            title="Projection Activation",
            metric="magneto_projection_strength",
    ...
    improved_row = _top_row(combined_aggregated, "magneto_hamiltonian_adam_improved")
    current_row = _top_row(combined_aggregated, "magneto_hamiltonian_adam")
    ...
    current_full_score = float(
        ablation_summary.loc[ablation_summary["variant_name"] == "current_magneto_full", "mean_selection_score"].iloc[0]
    ) if "current_magneto_full" in set(ablation_summary.get("variant_name", [])) else float("nan")
    ...
    improved_direct = win_flags.loc[win_flags["optimizer"] == "magneto_hamiltonian_adam_improved"] if not win_flags.empty else pd.DataFrame()
    improved_vs_current_row = (
        improved_direct.loc[improved_direct["baseline"] == "magneto_hamiltonian_adam"].head(1)
    ...
    report_lines = [
        "# Magneto-Hamiltonian Adam GPU Audit",
        "",
        "## Related Work",
        *[f"- [{entry['title']}]({entry['url']})" for entry in RELATED_WORK],
        ...
        "",
        "## Direct Answers",
        f"- Is Magneto-Hamiltonian GPU-capable? `{'yes' if answers['gpu_capable'] else 'not verified'}`",
    ...
    (output_path / "final_magneto_gpu_report.md").write_text("\n".join(report_lines), encoding="utf-8")
    return {
