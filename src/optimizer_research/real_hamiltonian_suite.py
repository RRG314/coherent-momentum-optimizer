...
def run_real_hamiltonian_smoke(config: dict[str, Any]) -> pd.DataFrame:
    return run_smoke_suite(config)


def run_real_hamiltonian_tuning(config: dict[str, Any]) -> pd.DataFrame:
    return run_tuning_suite(config)


def run_real_hamiltonian_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    return run_benchmark_suite(config)


def run_real_hamiltonian_energy_tests(config: dict[str, Any]) -> pd.DataFrame:
...
def run_real_hamiltonian_ablation(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "cpu")))
    seeds = list(config.get("seeds", [11, 29, 47]))
    task_names = list(
    ...
    variants: list[dict[str, Any]] = [
        {"variant_name": "real_full", "optimizer_name": "real_hamiltonian_adam", "overrides": {}},
        {
        ...
        for variant in variants:
            for seed in seeds:
                row = _train_single_run(
                    suite_name="real_hamiltonian_ablation",
                    task_name=task_name,
...
def _competitive_vs_rmsprop(aggregated: pd.DataFrame, optimizer_name: str = "real_hamiltonian_adam") -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
...
def export_real_hamiltonian_report(output_dir: str | Path) -> dict[str, Any]:
    ...
    real_vs_v1 = compute_meaningful_wins(combined_aggregated, "real_hamiltonian_adam", "hamiltonian_adam")
    real_vs_adamw = compute_meaningful_wins(combined_aggregated, "real_hamiltonian_adam", "adamw")
    real_vs_rmsprop = compute_meaningful_wins(combined_aggregated, "real_hamiltonian_adam", "rmsprop")
    real_vs_topological = compute_meaningful_wins(combined_aggregated, "real_hamiltonian_adam", "topological_adam")
    competitive_vs_rmsprop = _competitive_vs_rmsprop(combined_aggregated)

    v1_row = _best_row_for_optimizer(combined_aggregated, "hamiltonian_adam")
    ...
    v1_energy_drift = float(energy_agg.loc[energy_agg["optimizer"] == "hamiltonian_adam", "mean_relative_energy_drift"].mean())
    real_energy_drift = float(energy_agg.loc[energy_agg["optimizer"] == "real_hamiltonian_adam", "mean_relative_energy_drift"].mean())
    ...
    _plot_metric(
        ...
        optimizers=["real_hamiltonian_adam", "hamiltonian_adam", "adamw", "rmsprop"],
    )
    _plot_metric(
        trace_frame,
    ...
    _plot_metric(
        ...
        optimizers=["real_hamiltonian_adam", "hamiltonian_adam"],
    )
    _plot_metric(
        trace_frame,
    ...
    _plot_metric(
        ...
        optimizers=["real_hamiltonian_adam", "rmsprop", "adamw", "sgd_momentum"],
    )
    energy_bar = energy_agg[energy_agg["optimizer"].isin(["real_hamiltonian_adam", "hamiltonian_adam", "adamw", "rmsprop"])][
    ...
    if not trace_frame.empty:
        real_trace = trace_frame[(trace_frame["optimizer"] == "real_hamiltonian_adam") & (trace_frame["event"] == "train")]
    ...
    mode_text = "unknown"
    real_benchmark_rows = benchmark_frame[benchmark_frame["optimizer"] == "real_hamiltonian_adam"]
    ...
    report_lines = [
        ...
        *[
        ...
        "- V1 was reactive AdamW damping. `HamiltonianAdamReal` adds explicit position-momentum dynamics, kinetic/potential tracking, adaptive diagonal mass from Adam's second moment, and a leapfrog/symplectic-Euler integrator.",
        "",
        ...
        "",
        "## 5. Energy drift compared to V1",
        f"- Mean relative energy drift on direct energy tests: V1 `{v1_energy_drift:.6f}` vs Real `{real_energy_drift:.6f}`",
        "",
        ...
        f"- Tasks where it beat V1 and AdamW while staying competitive with RMSProp: {', '.join(strong_win_tasks) if strong_win_tasks else 'none'}",
        "",
        ...
        "",
        "## 11. Whether to keep V1, keep Real HamiltonianAdam, or combine them",
        "- Keep both for now.",
        "- V1 remains a simpler reactive stability baseline.",
        "- Real HamiltonianAdam is the more mathematically faithful branch, but only keep pushing it if the energy-drift gains survive without giving back too much to RMSProp on practical ML tasks.",
        "",
        "## Best Rows",
        _markdown_table(
            pd.DataFrame(
                [
                    {"optimizer": "hamiltonian_adam_v1", **(v1_row or {})},
