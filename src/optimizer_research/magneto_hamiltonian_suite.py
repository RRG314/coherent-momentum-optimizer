...
from .real_hamiltonian_suite import RELATED_WORK, _best_row_for_optimizer, _competitive_vs_rmsprop
...
def run_magneto_hamiltonian_smoke(config: dict[str, Any]) -> pd.DataFrame:
    return run_smoke_suite(config)


def run_magneto_hamiltonian_tuning(config: dict[str, Any]) -> pd.DataFrame:
    return run_tuning_suite(config)


def run_magneto_hamiltonian_benchmarks(config: dict[str, Any]) -> pd.DataFrame:
    return run_benchmark_suite(config)


def run_magneto_hamiltonian_energy_tests(config: dict[str, Any]) -> pd.DataFrame:
    ...
    for task_name in task_names:
        for optimizer_name in optimizers:
            for seed in seeds:
                rows.append(
                    _train_single_run(
                        suite_name="magneto_hamiltonian_energy",
                        task_name=task_name,
...
def run_magneto_hamiltonian_ablation(config: dict[str, Any]) -> pd.DataFrame:
    output_dir = ensure_output_dir(config)
    device = resolve_device(str(config.get("device", "cpu")))
    seeds = list(config.get("seeds", [11, 29, 47]))
    task_names = list(
    ...
    variants: list[dict[str, Any]] = [
        {"variant_name": "combined_full", "optimizer_name": "magneto_hamiltonian_adam", "overrides": {}},
        {
            "variant_name": "hamiltonian_real_baseline",
            "optimizer_name": "real_hamiltonian_adam",
            "overrides": {},
        },
        {"variant_name": "magneto_baseline", "optimizer_name": "magneto_adam", "overrides": {}},
        {
            "variant_name": "lr_scale_only_magneto",
        ...
        {
            "variant_name": "damping_only_magneto",
            "optimizer_name": "magneto_hamiltonian_adam",
            "overrides": {
        ...
        {
            "variant_name": "projection_only_magneto",
            "optimizer_name": "magneto_hamiltonian_adam",
            "overrides": {
                "alignment_strength": 0.0,
                "coherence_strength": 0.0,
        ...
        for variant in variants:
            for seed in seeds:
                row = _train_single_run(
                    suite_name="magneto_hamiltonian_ablation",
                    task_name=task_name,
...
def _plot_ablation_chart(ablation_frame: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.title("Magneto-Hamiltonian Adam Ablation")
    plt.tight_layout()
    plt.savefig(output_path, dpi=180)
    plt.close()


def export_magneto_hamiltonian_report(output_dir: str | Path) -> dict[str, Any]:
    ...
    combo_vs_real = compute_meaningful_wins(combined_aggregated, "magneto_hamiltonian_adam", "real_hamiltonian_adam")
    combo_vs_magneto = compute_meaningful_wins(combined_aggregated, "magneto_hamiltonian_adam", "magneto_adam")
    combo_vs_v1 = compute_meaningful_wins(combined_aggregated, "magneto_hamiltonian_adam", "hamiltonian_adam")
    combo_vs_adamw = compute_meaningful_wins(combined_aggregated, "magneto_hamiltonian_adam", "adamw")
    combo_vs_rmsprop = compute_meaningful_wins(combined_aggregated, "magneto_hamiltonian_adam", "rmsprop")
    combo_vs_topological = compute_meaningful_wins(combined_aggregated, "magneto_hamiltonian_adam", "topological_adam")
    competitive_vs_rmsprop = _competitive_vs_rmsprop(combined_aggregated, optimizer_name="magneto_hamiltonian_adam")

    combo_row = _best_row_for_optimizer(combined_aggregated, "magneto_hamiltonian_adam")
    real_row = _best_row_for_optimizer(combined_aggregated, "real_hamiltonian_adam")
    magneto_row = _best_row_for_optimizer(combined_aggregated, "magneto_adam")
    ...
    combo_energy_drift = float(
        energy_agg.loc[energy_agg["optimizer"] == "magneto_hamiltonian_adam", "mean_relative_energy_drift"].mean()
    )
    real_energy_drift = float(
        energy_agg.loc[energy_agg["optimizer"] == "real_hamiltonian_adam", "mean_relative_energy_drift"].mean()
    )
    ...
    _plot_metric(
        ...
        optimizers=["magneto_hamiltonian_adam", "real_hamiltonian_adam", "magneto_adam", "rmsprop"],
    )
    _plot_metric(
        trace_frame,
        ...
        optimizers=["magneto_hamiltonian_adam", "real_hamiltonian_adam", "magneto_adam", "adamw"],
        event="val",
    )
    _plot_metric(
        trace_frame,
        ...
        optimizers=["magneto_hamiltonian_adam", "real_hamiltonian_adam", "hamiltonian_adam"],
    )
    _plot_metric(
        trace_frame,
        ...
        optimizers=["magneto_hamiltonian_adam", "magneto_adam"],
    )
    _plot_metric(
        trace_frame,
    ...
    _plot_metric(
        ...
        title="RMSProp vs Magneto-Hamiltonian",
        metric="train_loss",
        tasks=["small_batch_instability", "overfit_small_wine", "oscillatory_valley"],
        optimizers=["magneto_hamiltonian_adam", "rmsprop", "adamw", "real_hamiltonian_adam"],
    )
    energy_bar = energy_agg[
        energy_agg["optimizer"].isin(["magneto_hamiltonian_adam", "real_hamiltonian_adam", "adamw", "rmsprop"])
    ...
    report_lines = [
        "# Magneto-Hamiltonian Adam Report",
        "",
        "## Related Work",
        *[f"- [{entry['title']}]({entry['url']}): {entry['summary']}" for entry in RELATED_WORK],
        "- Gradient alignment and conflict methods in multitask optimization are the nearest conceptual relatives for the magneto controller; this branch is best viewed as a directional-coherence add-on to the real Hamiltonian optimizer rather than a novelty claim.",
        "",
        "## 1. What changed from Real HamiltonianAdam",
        "- `MagnetoHamiltonianAdam` keeps the stabilized real-Hamiltonian core and adds directional coherence signals: gradient-momentum cosine, force-momentum cosine, gradient history cosine, update history cosine, rotation score, and a bounded projection back toward the force direction during conflict.",
        "- The current branch also adds activation gating so the magneto controller can stay closer to the real-Hamiltonian core on ordinary tasks and only fully activate when conflict or rotation rises.",
        "",
        "## 2. Whether it beat the strengthened real Hamiltonian baseline",
        f"- Meaningful wins vs `real_hamiltonian_adam`: {int(combo_vs_real['win'].sum())}",
        "",
        "## 3. Whether it beat MagnetoAdam alone",
        f"- Meaningful wins vs `magneto_adam`: {int(combo_vs_magneto['win'].sum())}",
        "",
        ...
        f"- Tasks where it beat Real Hamiltonian and AdamW while staying competitive with RMSProp: {', '.join(strong_win_tasks) if strong_win_tasks else 'none'}",
        "",
        "## 6. Whether it beat Topological Adam",
        f"- Meaningful wins vs `topological_adam`: {int(combo_vs_topological['win'].sum())}",
        "",
        "## 7. Energy drift compared to the real Hamiltonian branch",
        f"- Mean relative energy drift on direct energy tests: Real `{real_energy_drift:.6f}` vs Magneto-Hamiltonian `{combo_energy_drift:.6f}`",
        "",
        ...
        "",
        "## 9. Which magneto component mattered most",
        ...
        _markdown_table(
            pd.DataFrame(
                [
                    {"optimizer": "magneto_hamiltonian_adam", **(combo_row or {})},
                    {"optimizer": "real_hamiltonian_adam", **(real_row or {})},
                    {"optimizer": "magneto_adam", **(magneto_row or {})},
    ...
    return {
        ...
        "combo_vs_magneto": combo_vs_magneto,
