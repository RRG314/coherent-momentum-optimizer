...
from .coherent_momentum_real_baseline import CoherentMomentumRealBaseline
...
class CoherentMomentumOptimizer(CoherentMomentumRealBaseline):
    def __init__(
        self,
        params,
        lr: float = 0.02,
        ...
        self._initialize_physical_optimizer("CoherentMomentumOptimizer")

    def _compute_coherence_controls(
        self,
        *,
        grad: torch.Tensor,
        ...
        projection_gate = clamp_scalar(
            (coherence_activation - float(projection_activation_threshold)) / max(1.0 - float(projection_activation_threshold), DEFAULT_EPS),
            0.0,
            1.0,
        )
        ...
        return {
            ...
            "coherence_activation": coherence_activation,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                controls = self._compute_coherence_controls(
                    grad=grad,
                    momentum=momentum,
                    prev_grad=prev_grad,
                ...
                diagnostics["coherence_projection_strength"].append(float(controls["projection_strength"]))
                diagnostics["coherence_friction_multiplier"].append(float(controls["friction_multiplier"]))
        ...
        diagnostics_row = {
            ...
            "coherence_projection_strength": average(diagnostics["coherence_projection_strength"]),
            "coherence_friction_multiplier": average(diagnostics["coherence_friction_multiplier"]),
        }
        for key in [
            "grad_momentum_cosine",
            "force_momentum_cosine",
