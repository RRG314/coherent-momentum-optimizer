...
from .block_direction_optimizer_v4_fast import BlockDirectionOptimizerV4Fast, V4_COST_PENALTIES
from .optimizer_utils import average, clamp_scalar


class BlockDirectionOptimizerV42(BlockDirectionOptimizerV4Fast):
    """V4.2 keeps V4Fast's core and uses conv structure as trust, not direction.

    The branch target is broader usability without losing the block branch's
    stress-task identity. Instead of adding another default candidate, V4.2:

    - keeps the small V4Fast candidate set
    - keeps conv-safe scaling from V4.1
    - computes conv structure support from filter/channel/spatial consistency
    - uses that support to boost existing consensus/memory candidates
    - relaxes fallback slightly on coherent conv filters so the non-gradient
      block logic can actually engage on CNN tasks
    """
    ...
    def __init__(
        ...
        params,
        *,
        conv_consensus_bonus: float = 0.12,
        conv_memory_bonus: float = 0.08,
        conv_fallback_relaxation: float = 0.05,
        ...
        **kwargs: Any,
        ...
        self._initialize_physical_optimizer("BlockDirectionOptimizerV4.2")
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            conv_consensus_bonus = float(group.get("conv_consensus_bonus", 0.12))
            conv_memory_bonus = float(group.get("conv_memory_bonus", 0.08))
            conv_fallback_relaxation = max(0.0, float(group.get("conv_fallback_relaxation", 0.05)))
            ...
            for param in group["params"]:
                ...
                for candidate_name in self.candidate_names:
                    ...
                    candidate_conv_bonus = zero_metric
                    ...
                    selected_components[f"{candidate_name}__conv_support"] = conv_support.float() if conv_support is not None else zero_metric
                ...
                effective_fallback_threshold = fallback_threshold
                if conv_support is not None:
                    effective_fallback_threshold = torch.clamp(
                        torch.full((block_count,), fallback_threshold, dtype=torch.float32, device=param.device)
                        - conv_fallback_relaxation * conv_support,
                        min=0.0,
                    )
                else:
