# Magneto-Hamiltonian Improvement Plan

Optimizer families and baseline names mentioned below are referenced in [../../REFERENCES.md](../../REFERENCES.md).

## Main objective
Keep Magneto-Hamiltonian Adam as a specialist optimizer for unstable gradient-direction regimes while making it:
- GPU-safe
...
- Preserve `MagnetoHamiltonianAdam` as the accepted reference implementation.
- Do not silently overwrite the existing method.

### 2. Add a separate improved branch
- Introduce `MagnetoHamiltonianAdamImproved`.
