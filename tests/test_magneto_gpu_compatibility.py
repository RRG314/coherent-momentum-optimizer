...
from optimizers import MagnetoHamiltonianAdamImproved  # noqa: E402
...
def test_gpu_step_changes_parameters_and_state_stays_on_device() -> None:
    ...
    optimizer = MagnetoHamiltonianAdamImproved(model.parameters(), lr=0.02, diagnostics_every_n_steps=4, preset="standard_safe")
...
def test_cpu_to_gpu_state_dict_load_works() -> None:
    ...
    cpu_optimizer = MagnetoHamiltonianAdamImproved(cpu_model.parameters(), lr=0.02)
    criterion = torch.nn.MSELoss()
    ...
    gpu_optimizer = MagnetoHamiltonianAdamImproved(gpu_model.parameters(), lr=0.02)
...
def test_gpu_smoke_has_no_nans() -> None:
    ...
    set_global_seed(9)
    model = TinyRegressor().to(device)
    optimizer = MagnetoHamiltonianAdamImproved(model.parameters(), lr=0.02, preset="stress_specialist")
    criterion = torch.nn.MSELoss()
    for _ in range(4):
        x, y = _make_batch(device)
        optimizer.zero_grad(set_to_none=True)
...
def test_diagnostics_can_be_disabled_and_throttled() -> None:
    set_global_seed(13)
    model = TinyRegressor()
    optimizer = MagnetoHamiltonianAdamImproved(
        model.parameters(),
        lr=0.02,
