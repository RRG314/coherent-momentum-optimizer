...
from optimizer_research.coherent_momentum_real_baseline_suite import (  # noqa: E402
...
from optimizers import CoherentMomentumRealBaseline  # noqa: E402
...
def test_coherent_momentum_real_baseline_initializes_and_steps() -> None:
    set_global_seed(5)
    model = TinyRegressor()
    optimizer = CoherentMomentumRealBaseline(model.parameters(), lr=0.05)
...
def test_real_baseline_leapfrog_closure_recomputes_gradient() -> None:
    set_global_seed(11)
    model = TinyRegressor()
    optimizer = CoherentMomentumRealBaseline(model.parameters(), lr=0.03, mode="leapfrog_with_closure")
    criterion = torch.nn.MSELoss()
    x, y = _make_batch()
    optimizer.zero_grad(set_to_none=True)
    loss = criterion(model(x), y)
...
def test_real_baseline_leapfrog_without_closure_falls_back_cleanly() -> None:
    set_global_seed(13)
    model = TinyRegressor()
    optimizer = CoherentMomentumRealBaseline(model.parameters(), lr=0.03, mode="leapfrog_with_closure")
    criterion = torch.nn.MSELoss()
...
def test_real_baseline_no_nans_and_state_dict_round_trip() -> None:
    set_global_seed(17)
    model = TinyRegressor()
    optimizer = CoherentMomentumRealBaseline(model.parameters(), lr=0.03, mode="dissipative_hamiltonian")
    criterion = torch.nn.MSELoss()
    for _ in range(4):
        x, y = _make_batch()
        optimizer.zero_grad(set_to_none=True)
    ...
    reloaded_optimizer = CoherentMomentumRealBaseline(reloaded_model.parameters(), lr=0.03, mode="dissipative_hamiltonian")
    reloaded_optimizer.load_state_dict(state)
    assert "physical_global_state" in reloaded_optimizer.state_dict()


def test_real_baseline_diagnostics_exist() -> None:
    set_global_seed(23)
    model = TinyRegressor()
    optimizer = CoherentMomentumRealBaseline(model.parameters(), lr=0.03)
    criterion = torch.nn.MSELoss()
    x, y = _make_batch()
    optimizer.zero_grad(set_to_none=True)
    loss = criterion(model(x), y)
...
def test_real_baseline_benchmark_schema(tmp_path: Path) -> None:
    row = _train_single_run(
        suite_name="real_baseline_schema",
...
def test_real_baseline_ablation_schema(tmp_path: Path) -> None:
    frame = run_coherent_momentum_real_baseline_ablation(
        {
...
def test_real_baseline_report_generation(tmp_path: Path) -> None:
    rows = pd.DataFrame(
        [
            {
                "task": "demo_task",
                ...
                "final_val_loss": 0.5 if name != "coherent_momentum_real_baseline" else 0.4,
                "best_val_loss": 0.5 if name != "coherent_momentum_real_baseline" else 0.4,
                ...
                "mean_relative_energy_drift": 0.01 if name == "coherent_momentum_real_baseline" else 0.03,
                ...
                "seed": 11,
                "selection_score": -0.4 if name == "coherent_momentum_real_baseline" else -0.5,
                "trace_path": None,
            }
            for name in ["coherent_momentum_physical_baseline", "coherent_momentum_real_baseline", "adamw", "rmsprop", "topological_adam"]
        ]
    )
    ...
    export_coherent_momentum_real_baseline_report(tmp_path)
    assert (tmp_path / "final_report.md").exists()


def test_real_baseline_no_hardcoded_wins() -> None:
    aggregated = aggregate_results(
        pd.DataFrame(
            [
                {
                    "task": "demo",
                    "optimizer": "coherent_momentum_real_baseline",
                    "task_family": "demo",
                    "problem_type": "regression",
                    "final_val_loss": 1.2,
                    "best_val_loss": 1.2,
    ...
    wins = compute_meaningful_wins(aggregated, "coherent_momentum_real_baseline", "adamw")
