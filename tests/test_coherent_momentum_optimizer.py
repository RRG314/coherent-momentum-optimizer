...
from optimizer_research.coherent_momentum_suite import (  # noqa: E402
...
def test_coherence_hamiltonian_initializes_and_steps() -> None:
    set_global_seed(5)
    model = TinyRegressor()
...
def test_coherent_momentum_aliases_import_and_step() -> None:
    ...
    after = [param.detach().clone() for param in model.parameters()]
    assert any(not torch.allclose(a, b) for a, b in zip(before, after))
    assert isinstance(improved, CoherentMomentumOptimizerImproved)


def test_coherence_hamiltonian_improved_initializes_and_steps() -> None:
    set_global_seed(7)
    model = TinyRegressor()
    optimizer = CoherentMomentumOptimizerImproved(model.parameters(), lr=0.03, diagnostics_every_n_steps=2)
    criterion = torch.nn.MSELoss()
    x, y = _make_batch()
    before = [param.detach().clone() for param in model.parameters()]
    optimizer.zero_grad(set_to_none=True)
    ...
    assert latest["optimizer"] == "CoherentMomentumOptimizerImproved"
    assert latest["step"] == 1


def test_coherence_hamiltonian_neutral_is_close_to_real_baseline() -> None:
    set_global_seed(11)
    x, y = _make_batch()
    criterion = torch.nn.MSELoss()
    ...
    opt_a = CoherentMomentumRealBaseline(model_a.parameters(), lr=0.03)
    opt_b = CoherentMomentumOptimizer(
        model_b.parameters(),
        lr=0.03,
...
def test_coherence_hamiltonian_leapfrog_closure_recomputes_gradient() -> None:
    set_global_seed(13)
    model = TinyRegressor()
    optimizer = CoherentMomentumOptimizer(model.parameters(), lr=0.03, mode="leapfrog_with_closure")
    criterion = torch.nn.MSELoss()
    x, y = _make_batch()
    optimizer.zero_grad(set_to_none=True)
    loss = criterion(model(x), y)
...
def test_coherence_hamiltonian_no_nans_and_state_dict_round_trip() -> None:
    set_global_seed(17)
    model = TinyRegressor()
    optimizer = CoherentMomentumOptimizer(model.parameters(), lr=0.03)
    criterion = torch.nn.MSELoss()
    ...
    reloaded_optimizer = CoherentMomentumOptimizer(reloaded_model.parameters(), lr=0.03)
    reloaded_optimizer.load_state_dict(state)
    assert "physical_global_state" in reloaded_optimizer.state_dict()


def test_coherence_hamiltonian_diagnostics_exist() -> None:
    set_global_seed(23)
    model = TinyRegressor()
    optimizer = CoherentMomentumOptimizer(model.parameters(), lr=0.03)
    criterion = torch.nn.MSELoss()
    ...
    expected = {
        ...
        "alignment_scale",
        "coherence_projection_strength",
    }
    assert expected.issubset(latest.keys())


def test_coherence_hamiltonian_benchmark_schema(tmp_path: Path) -> None:
    ...
    expected = {
        ...
        "mean_rotation_score",
        "mean_coherence_activation",
    }
    assert expected.issubset(row.keys())


def test_coherent_momentum_ablation_schema(tmp_path: Path) -> None:
    frame = run_coherent_momentum_ablation(
        {
...
def test_coherence_hamiltonian_report_generation(tmp_path: Path) -> None:
    rows = pd.DataFrame(
        [
            {
                "task": "demo_task",
                ...
                "final_val_loss": 0.5 if name != "coherent_momentum_optimizer" else 0.35,
                "best_val_loss": 0.5 if name != "coherent_momentum_optimizer" else 0.35,
                ...
                "mean_relative_energy_drift": 0.005 if name == "coherent_momentum_optimizer" else 0.02,
                ...
                "mean_alignment_scale": 1.02 if name == "coherent_momentum_optimizer" else 1.0,
                "mean_grad_momentum_cosine": 0.2 if name == "coherent_momentum_optimizer" else float("nan"),
                "mean_rotation_score": 0.1 if name == "coherent_momentum_optimizer" else float("nan"),
                "seed": 11,
                "selection_score": -0.35 if name == "coherent_momentum_optimizer" else -0.5,
                "trace_path": None,
            }
            for name in [
    ...
    export_coherent_momentum_report(tmp_path)
    assert (tmp_path / "final_report.md").exists()


def test_coherence_hamiltonian_no_hardcoded_wins() -> None:
    aggregated = aggregate_results(
        pd.DataFrame(
            [
                {
                    "task": "demo",
                    "optimizer": "coherent_momentum_optimizer",
                    "task_family": "demo",
                    "problem_type": "regression",
                    "final_val_loss": 1.2,
                    "best_val_loss": 1.2,
    ...
    wins = compute_meaningful_wins(aggregated, "coherent_momentum_optimizer", "adamw")
